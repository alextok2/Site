...
function doLogin() {
    ...
    let password = MD5(document.getElementById("login").value+document.getElementById("password").value)
...
function checkPassword() {
    let indicator = document.getElementsByClassName("passwordIndicator")[0];
    ...
    if (!password.match(/.*\d/)) {
        console.log()
        message.innerText = "Должен содержать цифры"
    }
    else if (!password.match(/.*[a-zа-я]/)) {
        message.innerText = "Должен содержать строчные буквы"
    }
    else if (!password.match(/.*[A-ZА-Я]/)) {
        message.innerText = "Должен содержать заглавные буквы"
    }
    else if (password.length < 8) {
        message.innerText = "Должен содержать не менее 8 символов"
    }
    else if (!password.match(/^[а-яА-Я0-9a-zA-Z]{8,}$/)) {
        message.innerText = "Введены запретные символы"
    }
    /*(!password.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[а-яА-Я0-9a-zA-Z!@]{8,}$/)){
        message.innerText = "Че-то не так"
    }*/
    else{
        message.innerText = ""
...
function doRegister(func){
    let login = document.getElementById("login").value;
    let password = MD5(login+document.getElementById("password").value)
