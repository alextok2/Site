...
class NewTestForm(FlaskForm):
    ...
    name = StringField('Наименованние теста', validators=[DataRequired()], render_kw=style)
    question = TextAreaField('Вопрос', validators=[DataRequired()], render_kw=style)
    
    answer0 = StringField('Ответ 1', validators=[DataRequired()], render_kw=style)
    answer1 = StringField('Ответ 2', render_kw=style)
